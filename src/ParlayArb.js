...
class ParlayArb extends React.Component {

    constructor(props) {
        super(props);
        const storedArbs = localStorage.getItem("arbs");
    ...
    getArbMultiplier = (arb) => {
        let originalMultiplier = 1;
        if (arb.events.length) {
            arb.events.forEach((event, index) => {
                if (event.odd > 1) {
                    originalMultiplier = originalMultiplier * event.arb;
                }
            });
        }
        return originalMultiplier;
    ...
    render = () => {
        ...
        localStorage.setItem("arbs", JSON.stringify(arbs));
        let events = "";
        const tables = arbs.map((arb, arbIndex) => {
            ...
            if (arb.events && arb.events.length > 0) {
                const arbMult = this.getArbMultiplier(arb);
                const arbUnitStake = arbMult / arb.events.length;
                ...
                events = arb.events.map((event, index) => {
                    ...
                    if (event.arb > 0) {
                        // event.arbStake = Math.round(100 * shouldWin / (arbUnitStake -1)) / 100;
                    } else {
